...
const protect = async (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(token, process.env.JWT_SECRET || 'cravecrew_secret_key_2024');
...
const chefOnly = (req, res, next) => {
  if (req.user.role !== 'chef') return res.status(403).json({ message: 'Chef access only' });
  next();
};
